...
import { LayoutDashboard, FileText, Images, MessageSquare, Users, BarChart3, Settings, Shield, Zap, PlusCircle, LogOut } from "lucide-react";

const NAV = [
  { label: "Dashboard", href: "/admin", icon: <LayoutDashboard size={17} /> },
  { label: "Posts", href: "/admin/posts", icon: <FileText size={17} />, badge: "245" },
  { label: "Images", href: "/admin/images", icon: <Images size={17} /> },
  { label: "Comments", href: "/admin/comments", icon: <MessageSquare size={17} />, badge: "12" },
      ...
      <aside style={{ width: "240px", background: "var(--bg-secondary)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 100, flexShrink: 0 }}>
        ...
        <div style={{ padding: "1.25rem 1.25rem 1rem", borderBottom: "1px solid var(--border)" }}>
          <Link href="/admin" style={{ display: "flex", alignItems: "center", gap: "0.5rem", textDecoration: "none" }}>
            ...
            <div>
              <span style={{ fontWeight: 700, color: "#fff", fontSize: "0.9375rem" }}>PromptNest</span>
          ...
          {NAV.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}
                ...
                {item.badge && (
                  <span style={{ padding: "0.1rem 0.4rem", borderRadius: "100px", background: "rgba(255,49,49,0.2)", color: "#ff9eaa", fontSize: "0.7rem", fontWeight: 700 }}>
                    {item.badge}
                  </span>
                )}
              </Link>
        ...
        <div style={{ padding: "0.875rem 1rem", borderTop: "1px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "0.75rem" }}>
            ...
            <div>
              ...
              <p style={{ fontSize: "0.6875rem", color: "var(--text-tertiary)" }}>z••••••••@gmail.com</p>
            </div>
          </div>
          <div style={{ display: "flex", gap: "0.5rem" }}>
            ...
            <button className="btn btn-ghost" style={{ padding: "0.4rem 0.625rem", border: "1px solid var(--border)", color: "#ef4444" }}>
              <LogOut size={14} />
            </button>
          </div>
        </div>
